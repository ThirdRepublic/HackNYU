...
function test_function(){*/
	if ($_POST["mode"]==1){
		$cmd = "SELECT a.oh_ID AS OHID, a.oh_date AS OHDate, COUNT(a.app_ID) AS Frequency FROM appointment a INNER JOIN office_hour o ON o.oh_ID = a.oh_ID WHERE o.class_ID = ".$_POST['select']." GROUP BY a.oh_ID, a.oh_date ORDER BY OHDate";
		$statement = $conn->prepare($cmd);
		$statement->execute();
		$return = array();
		while($result = $statement->fetch()){
			$return[] = array("OHDate"=>$result['OHDate'], "Frequency"=>$result['Frequency']);
